...
def drawPause():
    ...
    resumeButton = drawCenteredButton(width/2, height/2-90, "Resume", comicSans, BLACK, WHITE, 10)
...
def drawSlider(x, y, length, currentXOffset, padding, min, max):
    ...
    leftSize = comicSans.size(str(min))
    rightSize = comicSans.size(str(max))
    leftText = comicSans.render(str(min), 1, BLACK)
    rightText = comicSans.render(str(max), 1, BLACK)
...
def drawSettings(radiusSetting, lengthSetting):
    ...
    # Ball radius 1-100
    display.blit(*drawCenteredText(width/2, 120, "Ball Radius (px):", comicSans, BLACK))
    ...
    display.blit(*drawCenteredText(width/2, 190, "Paddle Length (px):", comicSans, BLACK))
    paddleLengthSlider = drawSlider(width/2, 220, 100, lengthSetting, 10, paddleRange[MIN], paddleRange[MAX])  # accesing global scope maybe fix later

    return ballRadiusSlider, paddleLengthSlider
...
while running:
    ...
    if window == "Game":
        ...
        drawGame(guestY, hostY, paddleLength, ballX, ballY, ballRadius)
        radiusSlider, paddleSlider = drawSettings(radiusOffset, paddleOffset)
