...
running = True
sliderInFocus = [False, False, False]
...
def drawSettings(radiusSetting, lengthSetting, speedSetting, gamemode):
    display.blit(*drawCenteredText(width/2, 50, "SETTINGS", impact70, BLACK))

    # Ball radius 1-100
    display.blit(*drawCenteredText(width/2, 120, "Ball Radius (px):" + "  " + str(ballRadius), comicSans20, BLACK))
    ...
    display.blit(*drawCenteredText(width/2, 230, "Paddle Length (px):" + "  " + str(paddleLength), comicSans20, BLACK))
    ...
    # Ball Speed 1-20
    display.blit(*drawCenteredText(width/2, 340, "Ball Speed (px/frame):" + "  " + str(ballSpeed), comicSans20, BLACK))
    ballSpeedSlider = drawSlider(width/2, 370, 100, speedSetting, 10, speedRange[MIN], speedRange[MAX])

    gamemodeButton  = drawCenteredButton(width/2, 450, gamemode, comicSans20, BLACK, WHITE, 5)
    ...
    resetSpeed = drawCenteredButton(width/2, 400, "Reset", comicSans10, BLACK, WHITE, 5)

    return ballRadiusSlider, paddleLengthSlider, ballSpeedSlider, backButton, resetRadius, resetLength, resetSpeed, gamemodeButton
...
def getNewGamemode(gamemode):
    newMode = ""
    if gamemode == "Velocity":
        newMode = "Acceleration"
    elif gamemode == "Acceleration":
        newMode = "Jerk"
    elif gamemode == "Jerk":
        newMode = "Velocity"
...
##            print ballVec
        ...
        drawGame(guestY, hostY, paddleLength, ballX, ballY, ballRadius)
        radiusSlider, paddleSlider, speedSlider, backButton, resetRadius, resetLength, resetSpeed, gamemodeButton = drawSettings(radiusOffset, paddleOffset, speedOffset, paddleMode)
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                if (isInside(radiusSlider) or sliderInFocus[RADIUS]) and event.buttons[0] == 1:
                    ...
                    pygame.mouse.set_visible(False)
                    speedOffset = speedOffset+event.rel[0]
                    speedOffset = constrainValue(speedOffset, 0, 100)
                ...
                if isInside(backButton):
                    ...
                    speedOffset = DEFAULT_SPEED
                elif isInside(gamemodeButton):
                    buttonPress.play()
                    paddleMode = getNewGamemode(paddleMode)
