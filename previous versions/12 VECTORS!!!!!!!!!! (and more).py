...
while running:
    ...
    if window == "Game":
        ...
        if math.atan(abs(float(ballVec.y)/ballVec.x)) > maxBallAngle:
            print "-------------"
            print math.atan(abs(float(ballVec.y)/ballVec.x)), maxBallAngle
            print ballVec
            ballVec = pygame.math.Vector2(ballVec.x, math.copysign(ballVec.x*math.tan(maxBallAngle), ballVec.y))
            print ballVec
